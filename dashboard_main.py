...
with col1:
    ...
    fundamentals = {
        ...
        'Patrimônio Líquido': format_currency(stock.info.get('bookValue')),
...
try:
    ...
    news = stock.news
    for item in news[:3]:  # Exibir as três últimas notícias
        st.markdown(f"**{item['title']}**")  # Título da notícia
        st.caption(f"Fonte: {item['publisher']} - {pd.to_datetime(item['providerPublishTime'], unit='s').strftime('%d/%m/%Y %H:%M')}")  # Fonte e data da publicação
        st.write(item['link'])  # Link para a notícia completa
except Exception as e:
    st.error(f"Erro ao carregar as notícias: {str(e)}")  # Exibir erro caso a obtenção das notícias falhe
